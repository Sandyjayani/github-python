...
def find_net_force(force):
    add_horizontal = 0
    add_vertical = 0
    for tuple in force:
        magnitude = tuple[0]
        angle_degree = tuple[1]
        ...
        add_vertical += vertical
    total_magnitude = sqrt(add_horizontal**2 + add_vertical**2)
    total_angle = tan(add_vertical / add_horizontal)
    total_angle = degrees(total_angle)
    return (round(total_magnitude,1), round(total_angle,1))
